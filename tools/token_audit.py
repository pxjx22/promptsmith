...
def is_reference_or_negative_callout(line: str) -> bool:
    ...
    meta_patterns = [
        ...
        r"\b(remove|removing|delete|deleting|drop)\b.*\b(think carefully|reasoning (written )?out|write reasoning out)\b",
...
def find_cruft(content: str, filename: str = "") -> list[dict]:
    ...
    scaffold_patterns = [
        ...
        (re.compile(r"\bthink (carefully|hard|deeply) before (answering|responding|you (answer|respond))\b", re.IGNORECASE), "Thinking instruction on an always-thinking model", "Delete; tune effort instead (Claude Opus 5.5: replies start sooner with no quality loss)"),
        (re.compile(r"\b(show|write out|explain) your (reasoning|chain of thought|thought process) (in|before|at the start of) (your|the) (response|answer|reply)\b", re.IGNORECASE), "Reasoning-in-response request", "Delete; read summarized thinking blocks (Claude Opus 5.5 can refuse with reasoning_extraction)"),
