...
- **Coming from thinking-disabled Opus 5:** start at `low` effort and measure. If time to first token still matters, the system prompt line "Answer directly without deliberating." cuts thinking further; check quality when you add it. Read responses by block `type`, since a reply may open with a `thinking` block.
- **Refusals:** classifiers cover biology, cybersecurity, and reasoning extraction. Declines come back as HTTP 200 with `stop_reason: "refusal"` and a `stop_details` category. Handle them and configure fallback (`fallbacks: "default"`, beta); `reasoning_extraction` declines are returned to you rather than retried. Finding vulnerabilities in source code is allowed.
...
- **Progress updates:** between tool calls these arrive as `thinking` blocks, empty unless `display: "updates"` is set, so a client that renders only `text` looks silent. If it must hand the user something verbatim mid-turn (e.g. a code snippet), give it a send-message tool reserved for that, declared from the first request. For predictable updates (one line of intent before the first tool call, a short recap at the end), ask in the system prompt. If it goes quiet, append a turn-scoped reminder (`clear_at: "next_user_message"`) such as "The user hasn't heard from you in a while — say in a few words what you're doing, then continue." Stop after two or three reminders.
